    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="font-medium text-red-600">User</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <div class="flex flex-col">
                <div class="grid grid-cols-3 rounded-sm bg-gray-200   sm:grid-cols-7">
                    ...
                    <div class="p-2 xl:p-5 col-span-2">
                        <h5 class="text-sm font-medium uppercase sm:text-base">Nama User</h5>
                    </div>
                    <div class="p-2 text-center xl:p-5">
                        <h5 class="text-sm font-medium uppercase sm:text-base">Username</h5>
                    </div>
                    <div class="p-2 text-center xl:p-5">
                        <h5 class="text-sm font-medium uppercase sm:text-base">Role</h5>
                    </div>
                ...
                @foreach ($users as $user)
                    <div class="grid grid-cols-3 border-b border-gray-300   sm:grid-cols-7">
                        ...
                        <div class="flex items-center justify-center p-2 xl:p-5">
                            <p
                                class="inline-flex rounded-full bg-opacity-10 px-3 py-1 text-sm font-medium {{ $user->role == 'admin' ? 'text-yellow-600 bg-yellow-100' : 'text-blue-600 bg-blue-100' }}">
                                {{ $user->role }}
                            </p>
                        </div>
                        ...
                        <div class="hidden items-center justify-center p-2 sm:flex xl:p-3 gap-4">
                            ...
                            @else
                                <button type="button" disabled
                                    class="inline-flex items-center justify-center gap-2 bg-gray-500 px-10 py-4 text-center font-medium text-white cursor-not-allowed lg:px-2 xl:px-5 rounded">
                                    <span>
                                        <svg class="fill-current" width="15" height="15"
                                            xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 512 512">
                            ...
                            @if (auth()->user()->id !== $user->id)
                                @if (auth()->user()->role !== 'admin' || $user->role !== 'admin')
                                    <form id="delete-user-{{ $user->id }}"
                                        action="{{ route('dashboard.users.destroy', ['user' => $user->id]) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="showModal('delete-user-{{ $user->id }}')"
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-base font-semibold text-gray-900" id="modal-title">Hapus User</h3>
                            <p class="text-sm text-gray-500 mt-2">Are you sure you want to delete this user? This action
                                cannot be undone.</p>
                        </div>
                    </div>
                </div>
