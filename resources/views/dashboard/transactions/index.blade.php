                    ...
                    <tbody>
                        ...
                        <tr>
                            <td class="border-b border-gray-200 px-4 py-5 pl-9 dark:border-gray-300dark xl:pl-11">
                                <h5 class="font-medium text-black dark:text-white">{{ $transaction->invoice_number }}</h5>
                            </td>
                            <td class="border-b border-gray-200 px-4 py-5 pl-9 dark:border-gray-300dark xl:pl-11">
                                <h5 class="font-medium text-black dark:text-white">{{ $transaction->order->member ? $transaction->order->member->full_name : "Non Member" }}</h5>
                                <p class="text-sm">Rp. {{ number_format($transaction->order->total_price, 0, ',', '.') }} ({{ $transaction->order->total_items }} items)</p>
                            </td>
                            <td class="border-b border-gray-200 px-4 py-5 dark:border-gray-300dark">
                                <p class="text-black dark:text-white">{{ \Carbon\Carbon::parse($transaction->order->order_date)->format('M d, Y') }}</p>
                            </td>
                            <td class="border-b border-gray-200 px-4 py-5 dark:border-gray-300dark">
                                <p
                            ...
                            <td class="border-b border-gray-200 px-4 py-5 dark:border-gray-300dark">
                                <div class="flex items-center space-x-3.5">
                                    <a href="{{ route('dashboard.transactions.show', ['transaction' => $transaction->id]) }}" class="hover:text-red-600">
                                    ...
                                    <form id="delete-transaction-{{ $transaction->id }}" action="{{ route('dashboard.transactions.destroy', ['transaction' => $transaction->id]) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="showModal('delete-transaction-{{ $transaction->id }}')" class="hover:text-red-600">
                                            <svg class="fill-current" width="18" height="18" viewBox="0 0 18 18"
                                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            ...
                            <p class="text-sm text-gray-500 mt-2">Are you sure you want to delete this transaction record? This action
                                cannot be undone.</p>
                        </div>
                    </div>
                </div>
