    ...
    <div class="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <nav>
            <ol class="flex items-center gap-2">
                ...
                <li class="font-medium text-red-600">Diskon</li>
            </ol>
        </nav>
    </div>
    <!-- Breadcrumb End -->
            ...
            <div class="flex flex-col">
                <div class="grid grid-cols-3 rounded-sm bg-gray-200 dark:bg-meta-4 sm:grid-cols-7">
                    <div class="p-2 text-center xl:p-5">
                        <h5 class="text-sm font-medium uppercase xsm:text-base">Nama Diskon</h5>
                    </div>
                    <div class="p-2 text-center xl:p-5">
                        <h5 class="text-sm font-medium uppercase xsm:text-base">Tipe</h5>
                    </div>
                    <div class="p-2 text-center xl:p-5">
                        <h5 class="text-sm font-medium uppercase xsm:text-base">Total Diskon</h5>
                    </div>
                    <div class="hidden p-2 text-center sm:block xl:p-5">
                        <h5 class="text-sm font-medium uppercase xsm:text-base">Mulai Dari</h5>
                    </div>
                    <div class="hidden p-2 text-center sm:block xl:p-5">
                        <h5 class="text-sm font-medium uppercase xsm:text-base">Berakhir Pada</h5>
                    </div>
                    <div class="hidden p-2 text-center sm:block xl:p-5">
                ...
                @foreach ($discounts as $discount)
                    <div class="grid grid-cols-3 border-b border-gray-300 dark:border-gray-300dark sm:grid-cols-7">
                        <div class="flex items-center justify-center p-2 xl:p-5">
                            <p class="font-medium text-black dark:text-white">{{ $discount->name }}</p>
                        </div>

                        <div class="flex items-center justify-center p-2 xl:p-5">
                            <p class="font-medium text-black dark:text-white">{{ $discount->type }}</p>
                        </div>

                        <div class="flex items-center justify-center p-2 xl:p-5">
                            <p class="font-medium {{ $discount->status == 'active' ? 'text-green-600' : 'text-red-600' }}">
                                {{ $discount->type == 'fixed' ? 'Rp. ' . number_format($discount->value, 2, ',', '.') : $discount->value . '%' }}
                            </p>
                        </div>

                        <div class="hidden items-center justify-center p-2 sm:flex xl:p-5">
                            <p class="font-medium text-black dark:text-white">{{ $discount->start_date }}</p>
                        </div>

                        <div class="hidden items-center justify-center p-2 sm:flex xl:p-5">
                            <p class="font-medium text-black dark:text-white">{{ $discount->end_date }}</p>
                        </div>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                            <h3 class="text-base font-semibold text-gray-900" id="modal-title">Hapus Diskon?</h3>
                            <p class="text-sm text-gray-500 mt-2">Are you sure you want to delete this discount? This action
                                cannot be undone.</p>
                        </div>
                    </div>
                </div>
